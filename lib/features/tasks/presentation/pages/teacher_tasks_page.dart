...
class _TeacherTasksScreenState extends ConsumerState<TeacherTasksScreen> {
  ...
  Widget build(BuildContext context) {
    print("TEACHER ID FROM SCREEN = ${widget.teacherId}");
...
// استدعاء الـ Providers (اللي فيها teacherTasksNotifierProvider و teacherClassesProvider)
import '../providers/providers.dart';
import '../providers/teacher_tasks_notifier.dart'; // استدعاء الـ Notifier نفسه

// استدعاء الـ Widgets الخاصة بالشاشة
import '../widgets/task_item_card.dart';
import '../widgets/add_task_button.dart';
import '../widgets/loading_view.dart';
import '../widgets/empty_view.dart';
...
class _TeacherTasksScreenState extends ConsumerState<TeacherTasksScreen> {
  ...
  void initState() {
    super.initState();
    // لا حاجة لاستدعاء loadSavedTasks هنا لأننا نعتمد على الباك بالكامل
    // البيانات ستأتي عبر fetchTasks داخل teacherClassesProvider
  }
  ...
  Widget build(BuildContext context) {
    ...
    // جلب الكلاسات من الباك
    final classesAsync = ref.watch(teacherClassesProvider(widget.teacherId));
    // جلب حالة التاسكات (اللي بتتحط عن طريق fetchTasks)
    final state = ref.watch(teacherTasksNotifierProvider);
    final notifier = ref.read(teacherTasksNotifierProvider.notifier);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Tasks List (From Backend via teacherTasksNotifierProvider)
              Expanded(
                child: Builder(
                  builder: (_) {
                    if (state.isLoading) return const LoadingView();
